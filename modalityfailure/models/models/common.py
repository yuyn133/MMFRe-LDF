...
class LocalPerceptionUnit(nn.Module):
    def __init__(self, dim_embed, k=3, s=1, p=1):
        super(LocalPerceptionUnit, self).__init__()
        self.dwc = nn.Conv2d(dim_embed, dim_embed, kernel_size=k, stride=s, padding=p, groups=dim_embed)
        self.relu = nn.ReLU()

    def forward(self, x):
        identity = x
        return self.relu(self.dwc(x)) + identity


class DCNBlock(nn.Module):
    def __init__(self, d_model, mlp_ratio=4, mlp_drop=0.1):
        ...
        # self.mlp = TransformerMLPWithConv(self.d_model, expansion=mlp_ratio, drop=mlp_drop)

    def forward(self, x):
        x = x.permute(0, 2, 3, 1)  # N, H, W, C
        x = x + self.ln_1(self.dcnv3(x))
        x = x + self.ln_2(self.mlp(x))
        x = x.permute(0, 3, 1, 2).contiguous()  # dim:[b, c, h, w]
        return x
...
class FeaEnhance(nn.Module):
    def __init__(self, inx, kernel_size=3):
        super(FeaEnhance, self).__init__()
        self.ind = inx
        self.rgb_w = ECABlock(kernel_size)
        self.ir_w = ECABlock(kernel_size)

    def forward(self, x):
        rgb = x[0]
        ir = x[1]

        rgb_w = self.rgb_w(rgb)
        fea_rgb = rgb * rgb_w
        ...
        if self.ind == 1:
            return fea_rgb, fea_ir
        else:
            return (fea_ir + fea_rgb) / 2


class FusionBlock(nn.Module):
    def __init__(self, d_model, inx=1, mlp_ratio=4, n_layer=1, mlp_drop=0.1):
        super(FusionBlock, self).__init__()
        self.d_model = int(d_model * 2)
        self.n_layer = n_layer
        self.enhancement = FeaEnhance(inx=inx)

        # self.leu_rgb = LocalPerceptionUnit(dim_embed=d_model)
        # self.leu_ir = LocalPerceptionUnit(dim_embed=d_model)


        self.dcnblock = nn.Sequential(*[DCNBlock(d_model=self.d_model, mlp_ratio=mlp_ratio, mlp_drop=mlp_drop)
                                        for layer in range(self.n_layer)])

        self.leu = LocalPerceptionUnit(dim_embed=self.d_model)
    ...
    def forward(self, x):
        ...
        rgb, ir = self.enhancement(x)

        b, c, h, w = rgb.shape

        # rgb_fea = self.leu_rgb(rgb)
        # ir_fea = self.leu_ir(ir)

        fea_cat = torch.cat((rgb, ir), dim=1)
        new_fea = self.dcnblock(fea_cat)
        new_fea = self.leu(new_fea)

        new_fea = new_fea.reshape(b, 2, c, h, w).contiguous()

        new_rgb_fea = new_fea[:, 0, :, :, :].contiguous()
        new_ir_fea = new_fea[:, 1, :, :, :].contiguous()
...
#     y = torch.randn(4, 1024, 20, 20)
#     l = [x, y]
#     net = FusionBlock(d_model=1024)
#     start = time.time()
#     out = net(l)
#     end = time.time()
#     total = sum(p.numel() for p in net.parameters())
#     print('module\'s parameters: ', total)
